...
public class CVService {
    ...
    public List<CVResponse> searchCV(String keyword, boolean matchAll) {
        if (!matchAll)
    ...
    public CVModel saveCvPDF(MultipartFile file) throws IOException {
        ...
        // Define the destination directory to copy the file
        String destinationDirectory = "C:\\Users\\Nizar Jr\\Downloads\\CVIndexing-master\\cv-indexer\\public\\";

        // Construct the path for the destination file
        Path destinationFilePath = Paths.get(destinationDirectory, cvFile.getName());

        // Copy the file to the destination directory
        Files.copy(cvFile.toPath(), destinationFilePath);
